...
Texture::Texture(const char* fname, int rows, int cols)
{
    ...
    // Open the bitmap
    std::ifstream fin(std::string("NonEuclidean/Textures/") + fname, std::ios::in | std::ios::binary);
    if (!fin)
    {
        texId = 0;
        return;
    ...
    if (is3D)
    {
        ...
        glTexImage3D(
            GL_TEXTURE_2D_ARRAY, 0, GL_RGB8, width / rows, height / cols, rows * cols, 0, GL_BGR, GL_UNSIGNED_BYTE,
            img);
    }
    else
    {
