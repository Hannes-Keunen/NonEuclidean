...
void RdwTest3::Load(PObjectVec& objs, PPortalVec& portals, Player& player)
{
    ...
    auto room1 = std::make_shared<Room>(9.0f, 6.0f);
    room1->AddPortal(*p1, Vector3(-3.5f, 1.0f, 2.999f));
    objs.push_back(room1);

    auto room2 = std::make_shared<Room>(9.0f, 6.0f);
    room2->pos = Vector3(10.0f, 0.0f, 0.0f);
    room2->AddPortal(*p3, Vector3(3.5f, 1.0f, 2.999f));
    objs.push_back(room2);

    auto corridor = std::make_shared<Corridor>(Corridor::C);
